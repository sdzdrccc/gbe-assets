...
- 清单文件命名 `assemblies/<kit>/<category>/<name>.json`（与 `kits/` 的 `kit/category/name` 同构，目录名不变式同源，§2.2 与 `PLAN.md` §5）。
- `transform` 单位：`position` 米 · `rotation` **弧度**（XYZ 顺次，与几何内核一致）· `scale` 等比标量。**位置与旋转只写实例变换，不写几何**。
...
**实施说明（实现契约校验器时定死的解释，2026-09-15 · v1.2 增补）**

上表八条判据里有五处措辞不足以下手实现，本批程序化构件落地时一次性定死如下。它们**不是新裁决，而是对既有条款的解释**；实现位于 `packages/schema/assembly.js`（回归用例 `packages/schema/test/assembly.js`，27 项）。

| # | 契约沉默处 | 定死的解释 |
|---|---|---|
| 1 | `transform.rotation` 的单位（§19.4 示例全为 `[0,0,0]`，未标注） | **弧度**，XYZ 顺次旋转（与几何内核 `rotXYZ`、glTF / three 一致）。校验器发现某分量恰为 90 / 180 / 270 时**报警**——契约沉默处的歧义不靠猜 |
| 2 | `mate_types` 缺失时如何判互认（§9 标其为可选字段） | **缺省 = 通配**，不作限制。一个被声明为可选的字段，选而不填只能理解为不设限 |
| 3 | §9.1 说 `attach` 是"**单向挂接**"，而本条要求 `mate_types` **互认**——两句直接冲突 | **`attach` 例外**：一端为 `attach` 时，只校验"挂接方的 `mate_types` 含对端 `type`"，并**跳过** `direction` 反向检查。否则装饰件永远挂不上去 |
| 4 | 本条 ⑥ 与榫卯冲突：任何柱状节点件的榫接都必然互嵌 | **两类嵌入豁免**：① 已直接对接的对（mated pair）之间的嵌入；② **完全落在第三件 AABB 内**的嵌入（收容式，不可见）。只有"非对接件之间、且溢出到节点件体外"的交叠才报警 |
| 5 | ⑦ 的"可追溯至 `ground-foot` 承接面"如何判定 | **种子** = 拥有 `ground-foot(-y)` 插槽且该插槽**未对接**者（等价于"它所处的地面就在它脚下"——清单是建筑片段，地形标高不在其管辖内）。**传播**两条：① A 的插槽世界向 `+y` 且与 B 的 `−y` 插槽对接 ⇒ A 托 B；② **本件没有任何 `ground-foot` 插槽**时（檩、瓦、装饰件按定义不可能自立），其落地由宿主提供 |

**逃生舱**：`assembly check --strict` 把全部警告升级为错误，供 CI 使用。默认档位忠于上表——④⑥⑧ 三条确实只警告，不做静默加严。

**节点件接法（本批实测推出的配套规则）**

构件要能互相插上，节点件（转角 / 交叉）的插槽必须落 `snap_m` 网格。两条路各自成立，实测结论如下：

| 接法 | 插槽位置 | 前提 | 适用 |
|---|---|---|---|
| **面接** | 邻件的终止面 | 节点件**半边长**落 `snap_m` 网格 ⇒ **最小边长 1.0 m** | 墙线、枋、阶条石等线性件的转角平接 |
| **枢接** | 节点件**轴心** | 轴心落 `snap_m` 网格即可 | 柱状节点件：角墩、望柱、柱子 |

推论一：**L 形转角件的翼厚必须 ≥ 1.0 m**（= `2·snap_m` 的整数倍），否则两翼中心线落不到定位网格 —— 翼厚 0.5 m 的 L 形件**在契约下不可自洽**。所以 `cn-ancient.wall.qiang-corner-a` 实现为**枢轴式角墩**，L 形由装配清单表达（粒度判据 2「结构可分离」）。

推论二：§9.1 中 `wall-line` 的"相邻件 `position` 差必须为 `wall_module_m` 的整数倍"**只约束同轴续接的墙段之间**；转角节点件（枢接）与墙端之间的距离由节点件尺寸决定，不受此约束——否则任何非 2 m 尺寸的转角件都落不了位。
